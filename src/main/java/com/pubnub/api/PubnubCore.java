...
import com.pubnub.interfaces.PubnubInterface;
...
abstract public class PubnubCore implements PubnubInterface {
    ...
    protected String getPubnubUrl(Result result) {
        ...
        if (result != null) {
            result.getConfig().origin = ORIGIN_STR.split("://")[1];
            result.getConfig().TLS = this.SSL;
            result.getConfig().uuid = this.UUID;
            result.getConfig().authKey = this.AUTH_STR;
        }
        return ORIGIN_STR;
    }
