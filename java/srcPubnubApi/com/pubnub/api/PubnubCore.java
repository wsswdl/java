...
abstract class PubnubCore implements PubnubInterface {
	...
	protected static String VERSION = "Hi";
	...
	protected String getPubnubUrl(Result result) {
		...
		if (result != null) {
			result.setOrigin(ORIGIN_STR.split("://")[1]);
			result.setTLSEnabled(this.SSL);
		}
		return ORIGIN_STR;
	}
	...
	protected void decryptJSONArray(JSONArray messages)
			throws JSONException, IllegalStateException, IOException, PubnubException {
	...
	protected Object _publish(Hashtable args, boolean sync) {
		...
		if (this.SECRET_KEY.length() > 0) {
			...
			try {
				...
				PubnubError pe = PubnubError.getErrorObject(PubnubError.PNERROBJ_ENCRYPTION_ERROR, 6,
						msgStr + " : " + e.toString());
		...
		class HistoryResponseHandler extends ResponseHandler {
			...
			public void handleResponse(HttpRequest hreq, String response) {
				...
				try {
					...
					cb.errorCallback(channel, getPubnubError(e, PubnubError.PNERROBJ_DECRYPTION_ERROR, 10,
							response + " : " + e.toString()));
				} catch (Exception e) {
					cb.errorCallback(channel, PubnubError.getErrorObject(PubnubError.PNERROBJ_DECRYPTION_ERROR, 11,
							response + " : " + e.toString()));
				}
	...
	protected Object _hereNow(String[] channels, String[] channelGroups, boolean state, boolean uuids,
		...
		result.setAuthKey(this.AUTH_STR);
		result.setOperation(OperationType.HERE_NOW_FOR_CHANNEL);
		...
		HttpRequest hreq = new HttpRequest(path, parameters, new ResponseHandler() {
			public void handleResponse(HttpRequest hreq, String response, Result result) {
				result.setType(ResultType.RESPONSE);
			...
			public void handleError(HttpRequest hreq, PubnubError error, Result result) {
				...
				cb.errorCallback(null, error, new Status(result));
			}
		}, result);
		result.setHreq(hreq);
	...
	protected boolean validateInput(String name, Object input, Callback callback) {
		...
		if (input instanceof String && ((String) input).length() == 0) {
			callback.errorCallback("", PubnubError.getErrorObject(PubnubError.PNERROBJ_INVALID_ARGUMENTS, 2,
	...
	protected Object _pamGrant(final String channel, String auth_key, boolean read, boolean write, int ttl,
		...
		sign_input += "channel=" + PubnubUtil.pamEncode(channel) + "&" + "pnsdk=" + PubnubUtil.pamEncode(getUserAgent())
				+ "&" + "r=" + r + "&" + "timestamp=" + timestamp + ((ttl >= -1) ? "&" + "ttl=" + ttl : "") + "&" + "w="
				+ w;
		...
		status.setUuid(this.UUID);
		status.setOperation(OperationType.GRANT);
		

		String[] urlComponents = { getPubnubUrl(status), "v1", "auth", "grant", "sub-key", this.SUBSCRIBE_KEY };
		...
		HttpRequest hreq = new HttpRequest(urlComponents, parameters, new ResponseHandler() {
			...
			public void handleError(HttpRequest hreq, PubnubError error, Result status) {
				status.setType(ResultType.ERROR);
